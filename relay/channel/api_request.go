...
// clientRequestContext returns the inbound request context so upstream work is
// canceled when the client disconnects. Total time limits use http.Client.Timeout
// (RELAY_TIMEOUT) — do not wrap WithTimeout+defer cancel here, or SSE body reads
// would be aborted as soon as Do* returns.
func clientRequestContext(c *gin.Context) context.Context {
	if c != nil && c.Request != nil {
...
func newUpstreamRequest(c *gin.Context, method, url string, body io.Reader) (*http.Request, error) {
	return http.NewRequestWithContext(clientRequestContext(c), method, url, body)
}
...
func doRequest(c *gin.Context, req *http.Request, info *common.RelayInfo) (*http.Response, error) {
	...
	if err != nil {
		...
		if mapped := mapUpstreamRequestError(err); mapped != nil {
			return nil, mapped
		}
...
// mapUpstreamRequestError classifies client disconnect and timeout so the
// controller skips channel retries and the billing defer refunds pre-consume once.
func mapUpstreamRequestError(err error) *types.NewAPIError {
	if err == nil {
		return nil
	}
	// Client gone (includes cancel during TTFB once request uses client context).
	...
	// RELAY_TIMEOUT / http.Client.Timeout / explicit deadline.
	...
	// Client.Timeout sometimes surfaces without wrapping DeadlineExceeded.
	msg := err.Error()
...
// wrapDoRequestError preserves *types.NewAPIError (skip-retry / status) through the call chain.
