...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		apiRouter.GET("/verification", middleware.EmailVerificationRateLimit(), middleware.TurnstileCheck("email_verification"), controller.SendEmailVerification)
		apiRouter.GET("/reset_password", middleware.CriticalRateLimit(), middleware.TurnstileCheck("password_reset"), controller.SendPasswordResetEmail)
		apiRouter.POST("/user/reset", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, controller.ResetPassword)
		// OAuth routes - specific routes must come before :provider wildcard
		apiRouter.GET("/oauth/state", middleware.CriticalRateLimit(), controller.GenerateOAuthCode)
		apiRouter.POST("/oauth/email/bind", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, middleware.TurnstileCheck("email_bind"), controller.EmailBind)
		...
		billingRoute.Use(middleware.UserAuth())
		{
			...
			billingRoute.GET("/top-up/orders/:order_id", controller.GetBillingTopupOrder)
		}
		billingAdmin := apiRouter.Group("/billing/admin")
		...
		{
			userRoute.POST("/register", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, middleware.TurnstileCheck("register"), controller.Register)
			userRoute.POST("/login", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, middleware.TurnstileCheck("login"), controller.Login)
			...
			selfRoute.Use(middleware.UserAuth())
			{
				...
				selfRoute.POST("/checkin", middleware.TurnstileCheck("checkin"), controller.DoCheckin)

				// Social share reward (MVP §9.5) — Turnstile required
		...
		optionRoute.Use(middleware.RootAuth())
		{
			...
			optionRoute.POST("/email-provider/test", controller.SendTransactionalEmailTest)
			...
			optionRoute.GET("/stripe/:environment/credentials", controller.GetStripeCredentials)
			optionRoute.PUT("/stripe/:environment/credentials", controller.UpdateStripeCredentials)
			optionRoute.DELETE("/stripe/:environment/credentials", controller.DeleteStripeCredentials)
