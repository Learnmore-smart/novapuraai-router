...
func init() {
	// NovaPura ships the modern default theme (web/default). Classic is legacy Semi UI.
...
// EmailVerificationEnabled default true for NovaPura production posture; local can disable in admin.
...
// AutomaticDisableChannelEnabled: NovaPura MVP recommends on; ops can still disable via options.
...
// RegisterPromoEnabled grants promo to first RegisterPromoMax verified users.
var RegisterPromoEnabled = true
var RegisterPromoMax = 1000
var RegisterPromoCNYYuan = 200.0
...
// RelayTimeout is the upstream HTTP client total timeout in seconds (headers + body/SSE).
// 0 means no client-level timeout. Env: RELAY_TIMEOUT (default 840).
