    ...
    () => [
      ...
      // Status column
      {
        ...
        cell: ({ row }) => {
          ...
          const cooldownUntil = channel.cooldown_until ?? 0
          const failureCount = channel.failure_count ?? 0
          const isCoolingDown = cooldownUntil > Math.floor(Date.now() / 1000)
          const displayLabel = isCoolingDown ? t('Cooldown') : label
          const displayVariant: StatusBadgeProps['variant'] = isCoolingDown
            ? 'warning'
            : config.variant
                ...
                <Tooltip>
                  ...
                  <TooltipContent side='top' className='max-w-xs'>
                    <div className='space-y-1 text-xs'>
                      ...
                      {failureCount > 0 ? (
                        <div>
                          {t('Consecutive failures')}: {failureCount}
                        </div>
                      ) : null}
                      {statusReason ? (
                        <div>
