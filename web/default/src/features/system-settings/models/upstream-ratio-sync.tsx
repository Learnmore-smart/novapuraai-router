  ...
  const performSync = useCallback(
    async (currentRatios: ParsedRatios): Promise<boolean> => {
      const modelNames = [
        ...
        ...Object.values(currentRatios).flatMap((values) =>
          Object.keys(values),
        ),
      ];
      if (getInvalidUpstreamModelName([...new Set(modelNames)])) {
        toast.error(t("Failed to sync prices"));
        return false;
      }
      ...
      const modelMissingOutput = getTokenModelMissingOutput(
        finalRatios.ModelRatio,
        finalRatios.CompletionRatio,
        finalRatios.ModelPrice,
      );
      if (modelMissingOutput) {
        toast.error(`${t("Output price")} · ${modelMissingOutput}`);
        return false;
      }
