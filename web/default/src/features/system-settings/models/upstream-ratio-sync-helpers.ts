...
export function getInvalidUpstreamModelName(
  modelNames: string[],
): string | null {
  return modelNames.find((name) => isInvalidModelName(name)) ?? null;
}

export function getTokenModelMissingOutput(
  modelRatios: Record<string, number | string>,
  completionRatios: Record<string, number | string>,
  modelPrices: Record<string, number | string>,
): string | null {
  for (const modelName of Object.keys(modelRatios)) {
    if (Object.hasOwn(modelPrices, modelName)) continue;
    if (!hasPositiveTokenOutput(modelRatios[modelName])) return modelName;
    if (!hasPositiveTokenOutput(completionRatios[modelName])) return modelName;
  }
  return null;
}
...
export function getEffectiveResolutionSelections(
  ...
  selections.forEach((selection) => {
    ...
    effectiveByKey.set(
      `${resolved.model}\u0000${resolved.ratioType}`,
      resolved,
    );
  });
...
export function applyResolutionSelections(
  ...
  selections.forEach((selection) => {
    applyResolutionSelectionToDraft(
      drafts,
      resolutions,
      differences,
      selection,
