...
export async function getMyShareSubmissions(): Promise<
  ApiResponse<ShareSubmission[]>
> {
  const res = await api.get('/api/user/share')
  return res.data
}

export async function submitShareReward(
  request: ShareSubmissionRequest,
  turnstileToken?: string
): Promise<ApiResponse<ShareSubmission>> {
  const res = await api.post('/api/user/share', request, {
