...
export function BillingHistoryDialog({
  ...
  const handleCompleteDialogOpenChange = (nextOpen: boolean) => {
      ...
      >
        <AlertDialogContent>
          ...
          <div className='space-y-2'>
            <Label htmlFor='manual-topup-reason'>{t('Reason')}</Label>
            <Textarea
              id='manual-topup-reason'
              value={completionReason}
              onChange={(event) => setCompletionReason(event.target.value)}
              placeholder={t('Describe how payment was verified')}
              maxLength={512}
              disabled={completing}
              autoFocus
            />
            <p className='text-muted-foreground text-xs'>
              {t('This reason is recorded in the admin audit log.')}
            </p>
          </div>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              disabled={completing || completionReason.trim().length === 0}
            >
