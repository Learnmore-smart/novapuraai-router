...
export async function getShareReviewQueue(
  page: number,
  pageSize: number,
  status = 'pending'
): Promise<ShareReviewQueueResponse> {
  const params = new URLSearchParams({
    p: String(page),
    page_size: String(pageSize),
    status,
  })
  const res = await api.get(`/api/user/share/queue?${params.toString()}`)
  return res.data
}

export async function reviewShareSubmission(
  id: number,
  approve: boolean,
  reason: string
): Promise<ApiResponse> {
  const res = await api.post(`/api/user/share/${id}/review`, {
    approve,
    reason,
  })
  return res.data
