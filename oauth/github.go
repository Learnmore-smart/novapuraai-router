...
func (p *GitHubProvider) IsEnabled() bool {
	// Fail closed when the Secret Manager / env secret is missing.
	return common.GitHubOAuthActive()
}
...
func (p *GitHubProvider) ExchangeToken(ctx context.Context, code string, c *gin.Context) (*OAuthToken, error) {
	...
	if !common.GitHubOAuthReady() {
		logger.LogError(ctx, "[OAuth-GitHub] ExchangeToken rejected: client id or GITHUB_OAUTH_CLIENT_SECRET is not configured")
