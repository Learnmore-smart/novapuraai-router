...
func GetTopUpInfo(c *gin.Context) {
	...
	data := gin.H{
		...
		"enable_stripe_topup":         isLegacyStripeTopUpEnabled(),
		"enable_stripe_product_topup": isProductStripeTopUpEnabled(),
...
const (
	adminCompleteTopUpTradeNoMaxLength = 255
	adminCompleteTopUpReasonMaxLength  = 512
)

func validateAdminCompleteTopupRequest(req *AdminCompleteTopupRequest) error {
	if req == nil {
		return errors.New("invalid request")
	}
	req.TradeNo = strings.TrimSpace(req.TradeNo)
	req.Reason = strings.TrimSpace(req.Reason)
	if req.TradeNo == "" || len(req.TradeNo) > adminCompleteTopUpTradeNoMaxLength {
		return errors.New("invalid trade number")
	}
	if req.Reason == "" || len(req.Reason) > adminCompleteTopUpReasonMaxLength {
		return errors.New("an audit reason is required")
	}
	return nil
}
...
func AdminCompleteTopUp(c *gin.Context) {
	...
	if err := validateAdminCompleteTopupRequest(&req); err != nil {
		common.ApiErrorMsg(c, "参数错误")
		return
	}
	...
	result, err := model.ManualCompleteTopUp(req.TradeNo)
	if err != nil {
		common.ApiError(c, err)
		return
	}
	if result == nil {
		common.ApiErrorMsg(c, "补单结果无效")
		return
	}
	...
	if !result.AlreadyCompleted {
		model.RecordTopupLog(
			result.UserId,
			fmt.Sprintf("管理员补单成功，充值金额: %v，支付金额：%f", logger.FormatQuota(result.QuotaAdded), result.PayMoney),
			c.ClientIP(),
			result.PaymentMethod,
			"admin",
		)
	}
	recordManageAuditFor(c, result.UserId, "user.topup_complete", map[string]interface{}{
		"trade_no":          req.TradeNo,
		"reason":            req.Reason,
		"quota_added":       result.QuotaAdded,
		"quota_before":      result.QuotaBefore,
		"quota_after":       result.QuotaAfter,
		"already_completed": result.AlreadyCompleted,
	})
	common.ApiSuccess(c, gin.H{"already_completed": result.AlreadyCompleted})
}
