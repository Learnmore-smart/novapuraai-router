...
func CreateUser(c *gin.Context) {
	...
	myRole := c.GetInt("role")
	if user.Role > myRole || (user.Role == myRole && myRole != common.RoleRootUser) {
...
func ManageUser(c *gin.Context) {
	...
	case "demote":
		if myRole != common.RoleRootUser && user.Role >= myRole {
			common.ApiErrorI18n(c, i18n.MsgUserNoPermissionHigherLevel)
			return
		}
		if user.Id == c.GetInt("id") && user.Role == common.RoleRootUser {
...
func EmailBind(c *gin.Context) {
	...
	// Email verified/bound → attempt first-N register promo
	if _, _, e := model.TryGrantRegisterPromo(user.Id); e != nil {
		common.SysLog("register promo after email bind: " + e.Error())
	}
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "",
