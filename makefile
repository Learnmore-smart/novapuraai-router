...
# 号池 (channel pool) packages exercised by dynamic tests
...
# Dynamic 号池 tests: selection (priority/weight/enabled) + disable/retry policy
test-haochi:
	@echo "Running 号池 (channel pool) tests..."
	@go test -count=1 -timeout 90s $(HAOCHI_TEST_PKGS) -run $(HAOCHI_TEST_RUN)
