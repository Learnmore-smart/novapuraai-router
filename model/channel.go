...
type Channel struct {
	...
	// CooldownUntil: unix seconds; channel skipped by router until this time (e.g. after 429).
	CooldownUntil int64 `json:"cooldown_until" gorm:"bigint;default:0;column:cooldown_until"`
	// FailureCount consecutive failures for ops visibility (not sole disable trigger).
	FailureCount int `json:"failure_count" gorm:"type:int;default:0;column:failure_count"`
	// add after v0.8.5
...
// PlaintextKey returns the decrypted channel key material for upstream use.
func (channel *Channel) PlaintextKey() string {
	plain, err := common.DecryptChannelKey(channel.Key)
	if err != nil {
		common.SysError("channel key decrypt failed id=" + fmt.Sprint(channel.Id) + ": " + err.Error())
		return channel.Key
	}
	return plain
}

// EncryptKeyForStorage encrypts Key in place before DB write.
...
func (channel *Channel) InCooldown() bool {
	if channel.CooldownUntil <= 0 {
		return false
	}
	return time.Now().Unix() < channel.CooldownUntil
}

// SetCooldown sets cooldown for ChannelCooldownSeconds from now.
func (channel *Channel) SetCooldown(seconds int) error {
	if seconds <= 0 {
		seconds = common.ChannelCooldownSeconds
	}
	until := time.Now().Unix() + int64(seconds)
	if err := DB.Model(channel).Updates(map[string]any{
		"cooldown_until": until,
		"failure_count":  gorm.Expr("failure_count + 1"),
	}).Error; err != nil {
		return err
	}
	CacheSetChannelCooldown(channel.Id, until)
	return nil
}

// ClearCooldown clears temporary cooldown after success.
func (channel *Channel) ClearCooldown() error {
	if err := DB.Model(channel).Updates(map[string]any{
		"cooldown_until": 0,
		"failure_count":  0,
	}).Error; err != nil {
	...
	CacheClearChannelCooldown(channel.Id)
	return nil
}
