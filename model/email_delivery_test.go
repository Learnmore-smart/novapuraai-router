...
func TestEmailDeliveryReservationIsUniqueAcrossProviders(t *testing.T) {
	...
	first, created, err := ReserveEmailDelivery(newEmailDeliveryFixture("event-key", EmailProviderSES))
	require.NoError(t, err)
	require.True(t, created)
	assert.Equal(t, EmailProviderSES, first.Provider)
	...
	assert.Equal(t, EmailProviderSES, duplicate.Provider)
...
func TestEmailDeliveryTerminalTransitionsClearQueuedPayload(t *testing.T) {
	...
	delivery, created, err := ReserveEmailDelivery(newEmailDeliveryFixture("sent-key", EmailProviderSES))
	require.NoError(t, err)
	require.True(t, created)
	require.NoError(t, MarkEmailDeliverySent(delivery.Id, "ses-message-id", now))

	stored, err := GetEmailDeliveryByKey("sent-key")
	require.NoError(t, err)
	assert.Equal(t, EmailDeliveryStatusSent, stored.Status)
	assert.Equal(t, "ses-message-id", stored.ProviderMessageId)
...
func TestEmailDeliveryRetryQueueKeepsOriginalProviderAndClassifiesSafety(t *testing.T) {
	...
	safeRetry, created, err := ReserveEmailDelivery(newEmailDeliveryFixture("safe-retry", EmailProviderSES))
	require.NoError(t, err)
	require.True(t, created)
	require.NoError(t, MarkEmailDeliveryRetryQueued(safeRetry.Id, EmailFailureTimeoutAmbiguous, &deadline))

	manualReview, created, err := ReserveEmailDelivery(newEmailDeliveryFixture("manual-review", EmailProviderSES))
	require.NoError(t, err)
	require.True(t, created)
	require.NoError(t, MarkEmailDeliveryRetryQueued(manualReview.Id, EmailFailureTimeoutAmbiguous, nil))
	...
	assert.Equal(t, "manual-review", summary.Latest.IdempotencyKey)

	rows, err := ListSafeRetryEmailDeliveries(now, 10)
	require.NoError(t, err)
	require.Len(t, rows, 1)
	assert.Equal(t, safeRetry.Id, rows[0].Id)
	assert.NotEmpty(t, rows[0].EncryptedPayload)

	claimed, err := ClaimEmailDeliveryForSafeRetry(safeRetry.Id, now)
	require.NoError(t, err)
	assert.True(t, claimed)
	claimedAgain, err := ClaimEmailDeliveryForSafeRetry(safeRetry.Id, now)
	require.NoError(t, err)
	assert.False(t, claimedAgain)

	stored, err := GetEmailDeliveryByKey("safe-retry")
	require.NoError(t, err)
...
func TestEmailDeliveryDefinitiveFailureCanBeClaimedByNewProvider(t *testing.T) {
	setupEmailDeliveryTest(t)
	delivery, created, err := ReserveEmailDelivery(newEmailDeliveryFixture("definitive-failure", EmailProviderSES))
	require.NoError(t, err)
	require.True(t, created)
