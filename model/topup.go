...
// balance transition required by the administrator audit log.
func ManualCompleteTopUp(tradeNo string) (*ManualTopUpResult, error) {
	if tradeNo == "" {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		// Idempotent retries are observable to the controller but never credit or
		// emit another user top-up log.
		...
		quotaToAdd, clamp := common.QuotaFromDecimalChecked(quotaDecimal.Truncate(0))
		if clamp != nil {
		...
		if user.Quota > common.MaxQuota-quotaToAdd {
			return errors.New("充值后用户余额超出支持范围")
		}
		...
		// The user row is locked above, so writing the exact audited post-balance
		// keeps the stored value and the audit transition identical.
		if err := tx.Model(&User{}).Where("id = ?", topUp.UserId).Update("quota", quotaAfter).Error; err != nil {
			return err
		}
