...
type User struct {
	...
	// RewardedInviteCount counts invite rewards already paid (cap common.MaxValidInvites).
	RewardedInviteCount int `json:"rewarded_invite_count" gorm:"type:int;default:0;column:rewarded_invite_count"`
	AffQuota            int `json:"aff_quota" gorm:"type:int;default:0;column:aff_quota"`           // 邀请佣金池（可转入现金）
...
func inviteUser(inviterId int) (err error) {
	// Legacy immediate path kept for compatibility when delayed invite is disabled.
	// Cap rewarded invites at MaxValidInvites (default 10).
	return DB.Transaction(func(tx *gorm.DB) error {
		...
		user.RewardedInviteCount++
		user.AffQuota += common.QuotaForInviter
		user.AffHistoryQuota += common.QuotaForInviter
		return tx.Model(&user).Updates(map[string]any{
			"aff_count":             user.AffCount,
			"rewarded_invite_count": user.RewardedInviteCount,
...
// CashQuota returns derived cash portion (total - promo). Never negative.
func (user *User) CashQuota() int {
	c := user.Quota - user.PromoQuota
	if c < 0 {
		return 0
	}
	return c
...
func (user *User) TransferAffQuotaToQuota(quota int) error {
	...
	// Transfer into cash (total Quota); promo unchanged
	user.AffQuota -= quota
	user.Quota += quota
...
func (user *User) Insert(inviterId int) error {
	if err := DB.Transaction(func(tx *gorm.DB) error {
		return withNormalizedEmailLock(tx, user.Email, func(tx *gorm.DB) error {
			...
			// New-user gift is promo (gift), not cash top-up.
...
func (user *User) finishInsert(inviterId int) {
	...
	if inviterId != 0 {
		// Block self-invite by id
		if inviterId == user.Id {
			return
		}
		if common.DelayedInviteReward {
			_ = DB.Model(&User{}).Where("id = ?", user.Id).Updates(map[string]any{
...
func IncreaseUserQuota(id int, quota int, db bool) (err error) {
	return IncreaseUserCashQuota(id, quota, db)
}

// IncreaseUserCashQuota adds real (cash) balance: quota += n, promo unchanged.
func IncreaseUserCashQuota(id int, quota int, db bool) (err error) {
	if quota < 0 {
	...
	gopool.Go(func() {
		if err := cacheIncrUserQuota(id, int64(quota)); err != nil {
			common.SysLog("failed to increase user cash quota cache: " + err.Error())
		}
	})
	...
	return increaseUserCashQuota(id, quota)
}

// IncreaseUserPromoQuota adds gift balance: quota += n and promo_quota += n.
func IncreaseUserPromoQuota(id int, quota int, db bool) (err error) {
	if quota < 0 {
		return errors.New("quota 不能为负数！")
	}
	if quota == 0 {
	...
	gopool.Go(func() {
		if err := cacheIncrUserQuota(id, int64(quota)); err != nil {
			common.SysLog("failed to increase user promo quota cache: " + err.Error())
		}
	})
	// Batch path only tracks total; promo applied immediately for correctness.
	return increaseUserPromoQuota(id, quota)
}

func increaseUserCashQuota(id int, quota int) (err error) {
	return DB.Model(&User{}).Where("id = ?", id).Update("quota", gorm.Expr("quota + ?", quota)).Error
}

func increaseUserPromoQuota(id int, quota int) (err error) {
	return DB.Model(&User{}).Where("id = ?", id).Updates(map[string]any{
		"quota":       gorm.Expr("quota + ?", quota),
		"promo_quota": gorm.Expr("promo_quota + ?", quota),
	}).Error
}
...
// DecreaseUserQuota deducts promo first, then cash. Never drives total below zero (atomic check).
func DecreaseUserQuota(id int, quota int, db bool) (err error) {
	_, err = DecreaseUserQuotaWithSplit(id, quota, db)
	return err
}

// DecreaseUserQuotaWithSplit is the dual-balance deduct used by billing for accurate refunds.
func DecreaseUserQuotaWithSplit(id int, amount int, db bool) (split QuotaWalletSplit, err error) {
	if amount < 0 {
		return split, errors.New("quota 不能为负数！")
	}
	if amount == 0 {
		return split, nil
	}
	// Always hit DB for split correctness (batch update cannot track promo/cash).
	split, err = decreaseUserQuotaWithSplit(id, amount)
	if err != nil {
		return split, err
	}
	gopool.Go(func() {
...
func decreaseUserQuota(id int, quota int) (err error) {
	_, err = decreaseUserQuotaWithSplit(id, quota)
	return err
}
...
func RestoreUserQuotaSplit(id int, split QuotaWalletSplit) error {
	if split.Promo < 0 || split.Cash < 0 {
		return errors.New("quota 不能为负数！")
	}
	...
	gopool.Go(func() {
		if e := cacheIncrUserQuota(id, int64(total)); e != nil {
			common.SysLog("failed to restore user quota cache: " + e.Error())
		}
	})
	return nil
}

// GetUserPromoQuota returns promo portion from DB.
func GetUserPromoQuota(id int) (int, error) {
	var promo int
	err := DB.Model(&User{}).Where("id = ?", id).Select("promo_quota").Find(&promo).Error
	return promo, err
}
